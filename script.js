document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        // Allow copy if text is highlighted (to permit native copy in contact section)
        if (isCopy && hasSelection) {
            return; 
        }

        // Prevent Ctrl+C (if no selection), Ctrl+X, Ctrl+U (View Source), Ctrl+S (Save)
        if (e.ctrlKey && ['c', 'C', 'x', 'X', 'u', 'U', 's', 'S'].includes(e.key)) {
            e.preventDefault();
        }
    });

    // 9. 3D Tilt Effect Initialization
